...
class API:
    ...
    def _build_params(self, values=[], conditions=[]):
        params_list = []
        for value_set in values:
    ...
    def get_list(self, table, conditions, limit=100, offset=0):
        data = self._select(table, conditions, limit, offset)
        if data is None:
            self.error_code = API.Code.ERR_DATA_FAILED
            self.error_message.add('check_exist', 'データベース操作にエラーが発生しました')
            return None
    ...
    def _select(self, table, conditions, limit, offset):
        sql = 'SELECT * FROM `{}` {} LIMIT {} OFFSET {}'.format(table, self._build_conditions(conditions), limit, offset)
