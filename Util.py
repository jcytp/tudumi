...
class Util:
    ...
    def generate_secret(secret, iterations_count=100):
        ...
        # return PBKDF2(secret, salt, 32, count=iterations_count, hmac_hash_module=SHA256)
        Util.SECRET_KEY = PBKDF2(secret, salt, 32, count=iterations_count, hmac_hash_module=SHA256)

    @staticmethod
    # def encrypt_text(secret, plain_text):
    ...
    @staticmethod
    # def decrypt_text(secret, cipher_text):
    def decrypt_text(cipher_text):
        try:
            ...
            # cipher = AES.new(secret, AES.MODE_CBC, iv_bytes)
    ...
    def b64_decode(b64text):
        return base64.b64decode(b64text.encode()).decode()
